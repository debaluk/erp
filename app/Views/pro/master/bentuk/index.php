    ...
    <div class="modal-dialog" role="document">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group row">
                    <label for="kategori" class="col-sm-3 col-form-label">Nama Bentuk</label>
                    <div class="col-sm-9">
                        <input type="text" class="form-control" name="nama_bentuk" id="nama_bentuk">
                        <small class="invalid-feedback"></small>
                    </div>
                </div>
...
<script>
    $(document).ready(function() {
        const table = $("#table-kategori").DataTable({
            ...
            order: [],
            ajax: {
                url: `${BASE_URL}/pro/master/bentuk/ajax`
            },
            //optional
            lengthMenu: [
                [10, 25,50,100],
            ...
            columns: [
                ...
                
                {
                    data: function(row) {
                        ...
                        let html = '<button class="btn btn-success btn-sm mr-1 ubah" data-id="' + row.id_bentuk + '" data-kode="' + row.kode + '" data-nama="' + row.nama + '"><i class="fas fa-edit"></i></button>'
                        html += '<button class="btn btn-danger btn-sm hapus" data-id="' + row.id_bentuk + '"><i class="fa fa-trash"></i></button>'
                        return html;
                    }
                }
            ],
        ...
        $(".content").on("click", "#tambah", function(e) {
            ...
            $.ajax({
                type: "post",
                url: `${BASE_URL}/pro/master/bentuk/tambah`,
                dataType: "json",
                contentType: false,
                processData: false,
                cache: false,
                ...
                success: function(response) {
                    responValidasi(['tambah'], ['kode', 'nama_bentuk'], response);
                    if (response.sukses) {
        ...
        $(".content").on("click", ".ubah", function() {
            ...
            $("#nama_bentuk").val($(this).data("nama"));
            $(".modal-footer").append('<input type="hidden" name="id_bentuk" value="' + $(this).data("id") + '">');
        })
        ...
        $(".content").on("click", "#ubah", function(e) {
            ...
            $.ajax({
                type: "post",
                url: `${BASE_URL}/pro/master/bentuk/ubah`,
                dataType: "json",
                contentType: false,
                processData: false,
                cache: false,
                data: formData,
                success: function(response) {
                    responValidasi(['ubah'], ['kode', 'nama_bentuk'], response);
                    if (response.sukses) {
                        $("#formModal").modal("hide");
                        table.ajax.reload();
                    }
        ...
        $(".content").on("click", ".hapus", function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `${BASE_URL}/pro/master/bentuk/hapus`,
                        data: {
                            id: $(this).data("id")
                        },
                        success: function(response) {
