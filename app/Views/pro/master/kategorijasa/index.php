        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        <input type="text" class="form-control" name="nama_kategori_jasa" id="nama_kategori_jasa">
                        <small class="invalid-feedback"></small>
                    </div>
                </div>
                <div class="form-group row">
...
<script>
    $(document).ready(function() {
        const table = $("#table-kategori").DataTable({
            ...
            order: [],
            ajax: {
                url: `${BASE_URL}/pro/master/kategorijasa/ajax`
            },
            //optional
            lengthMenu: [
                [10, 25,50,100],
            ...
            columns: [
                ...
                
                {
                    data: function(row) {
                        ...
                        let html = '<button class="btn btn-success btn-sm mr-1 ubah" data-id="' + row.kategori_jasa_id + '" data-kode="' + row.kode + '" data-nama="' + row.nama + '"data-keterangan="' + row.keterangan + '"><i class="fas fa-edit"></i></button>'
                        html += '<button class="btn btn-danger btn-sm hapus" data-id="' + row.kategori_jasa_id + '"><i class="fa fa-trash"></i></button>'
                        return html;
                    }
                }
            ],
        ...
        $(".content").on("click", "#tambah", function(e) {
            ...
            $.ajax({
                type: "post",
                url: `${BASE_URL}/pro/master/kategorijasa/tambah`,
                dataType: "json",
                contentType: false,
                processData: false,
                cache: false,
                ...
                success: function(response) {
                    responValidasi(['tambah'], ['kode', 'nama_kategori_jasa'], response);
                    if (response.sukses) {
        ...
        $(".content").on("click", ".ubah", function() {
            ...
            $("#nama_kategori_jasa").val($(this).data("nama"));
            $("#keterangan").val($(this).data("keterangan"));
            $(".modal-footer").append('<input type="hidden" name="kategori_jasa_id" value="' + $(this).data("id") + '">');
        })
        ...
        $(".content").on("click", "#ubah", function(e) {
            ...
            $.ajax({
                type: "post",
                url: `${BASE_URL}/pro/master/kategorijasa/ubah`,
                dataType: "json",
                contentType: false,
                processData: false,
                cache: false,
                data: formData,
                success: function(response) {
                    responValidasi(['ubah'], ['kode', 'nama_kategori_jasa'], response);
                    if (response.sukses) {
                        $("#formModal").modal("hide");
                        table.ajax.reload();
                    }
        ...
        $(".content").on("click", ".hapus", function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `${BASE_URL}/pro/master/kategorijasa/hapus`,
                        data: {
                            id: $(this).data("id")
                        },
                        success: function(response) {
