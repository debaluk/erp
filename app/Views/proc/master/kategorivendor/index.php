        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        <input type="text" class="form-control" name="kategori_vendor_nama" id="kategori_vendor_nama">
                        <small class="invalid-feedback"></small>
                    </div>
                </div>
                <div class="form-group row">
...
<script>
    $(document).ready(function() {
        const table = $("#table-kategori").DataTable({
            ...
            order: [],
            ajax: {
                url: `${BASE_URL}/proc/master/kategorivendor/ajax`
            },
            //optional
            lengthMenu: [
                [10, 25,50,100],
            ...
            columns: [
                ...
                {
                    data: 'kategori_vendor_nama',
                    name: 'kategori_vendor_nama'
                },
                {
                    data: 'keterangan',
                    name: 'keterangan'
                },
                {
                    data: function(row) {
                        ...
                        let html = '<button class="btn btn-success btn-sm mr-1 ubah" data-id="' + row.kategori_vendor_id + '" data-kode="' + row.kode + '" data-kategori_vendor="' + row.kategori_vendor_nama + '" data-keterangan="' + row.keterangan + '"><i class="fas fa-edit"></i></button>'
                        html += '<button class="btn btn-danger btn-sm hapus" data-id="' + row.kategori_vendor_id + '"><i class="fa fa-trash"></i></button>'
                        return html;
                    }
                }
            ],
        ...
        $(".content").on("click", "#tambah", function(e) {
            ...
            $.ajax({
                type: "post",
                url: `${BASE_URL}/proc/master/kategorivendor/tambah`,
                dataType: "json",
                contentType: false,
                processData: false,
                cache: false,
                ...
                success: function(response) {
                    responValidasi(['tambah'], ['kode', 'kategori_vendor_nama'], response);
                    if (response.sukses) {
        ...
        $(".content").on("click", ".ubah", function() {
            ...
            $("#kategori_vendor_nama").val($(this).data("kategori_vendor"));
            $("#keterangan").val($(this).data("keterangan"));
            $(".modal-footer").append('<input type="hidden" name="kategori_vendor_id" value="' + $(this).data("id") + '">');
        })
        ...
        $(".content").on("click", "#ubah", function(e) {
            ...
            $.ajax({
                type: "post",
                url: `${BASE_URL}/proc/master/kategorivendor/ubah`,
                dataType: "json",
                contentType: false,
                processData: false,
                cache: false,
                data: formData,
                success: function(response) {
                    responValidasi(['ubah'], ['kode', 'kategori_vendor_nama'], response);
                    if (response.sukses) {
                        $("#formModal").modal("hide");
                        table.ajax.reload();
                    }
        ...
        $(".content").on("click", ".hapus", function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `${BASE_URL}/proc/master/kategorivendor/hapus`,
                        data: {
                            id: $(this).data("id")
                        },
                        success: function(response) {
