...
<div class="container-fluid">
    <button class="btn btn-primary mb-1 tambah"><i class="fas fa-plus"></i> Tambah Satuan</button>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group row">
                    <label for="kategori" class="col-sm-3 col-form-label">Simbol</label>
                    <div class="col-sm-9">
...
<script>
    $(document).ready(function() {
        const table = $("#table-kategori").DataTable({
            ...
            order: [],
            ajax: {
                url: `${BASE_URL}/hrd/master/satuan/ajax`
            },
            //optional
            lengthMenu: [
                [10, 25,50,100],
            ...
            columns: [
                ...
                },
                {
                    data: function(row) {
                        ...
                        let html = '<button class="btn btn-success btn-sm mr-1 ubah" data-id="' + row.satuan_id + '" data-simbol="' + row.simbol + '" data-satuan="' + row.satuan + '" data-keterangan="' + row.keterangan + '"><i class="fas fa-edit"></i></button>'
                        html += '<button class="btn btn-danger btn-sm hapus" data-id="' + row.satuan_id + '"><i class="fa fa-trash"></i></button>'
                        return html;
                    }
                }
            ],
        ...
        $(".content").on("click", "#tambah", function(e) {
            ...
            $.ajax({
                type: "post",
                url: `${BASE_URL}/hrd/master/satuan/tambah`,
                dataType: "json",
                contentType: false,
                processData: false,
                cache: false,
                ...
                success: function(response) {
                    responValidasi(['tambah'], ['simbol', 'satuan'], response);
                    if (response.sukses) {
        ...
        $(".content").on("click", ".ubah", function() {
            ...
            // isi tiap kolom
            $("#simbol").val($(this).data("simbol"));
            $("#satuan").val($(this).data("satuan"));
            $("#keterangan").val($(this).data("keterangan"));
            $(".modal-footer").append('<input type="hidden" name="satuan_id" value="' + $(this).data("id") + '">');
        })
        ...
        $(".content").on("click", "#ubah", function(e) {
            ...
            $.ajax({
                type: "post",
                url: `${BASE_URL}/hrd/master/satuan/ubah`,
                dataType: "json",
                contentType: false,
                processData: false,
                cache: false,
                data: formData,
                success: function(response) {
                    responValidasi(['ubah'], ['simbol', 'satuan'], response);
                    if (response.sukses) {
                        $("#formModal").modal("hide");
                        table.ajax.reload();
                    }
        ...
        $(".content").on("click", ".hapus", function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `${BASE_URL}/hrd/master/satuan/hapus`,
                        data: {
                            id: $(this).data("id")
                        },
                        success: function(response) {
