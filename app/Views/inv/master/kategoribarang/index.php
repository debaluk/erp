...
<script>
    $(document).ready(function() {
        const table = $("#table-kategori").DataTable({
            ...
            order: [],
            ajax: {
                url: `${BASE_URL}/inv/master/kategoribarang/ajax`
            },
            //optional
            lengthMenu: [
                [10, 25,50,100],
            ...
            columns: [
                ...
                },
                {
                    data: function(row) {
                        ...
                        let html = '<button class="btn btn-success btn-sm mr-1 ubah" data-id="' + row.kategori_barang_id + '" data-kode="' + row.kode + '" data-kategori_barang="' + row.kategori_barang + '" data-keterangan="' + row.keterangan + '"><i class="fas fa-edit"></i></button>'
                        html += '<button class="btn btn-danger btn-sm hapus" data-id="' + row.kategori_barang_id + '"><i class="fa fa-trash"></i></button>'
                        return html;
                    }
                }
            ],
        ...
        $(".content").on("click", "#tambah", function(e) {
            ...
            $.ajax({
                type: "post",
                url: `${BASE_URL}/inv/master/kategoribarang/tambah`,
                dataType: "json",
                contentType: false,
                processData: false,
                cache: false,
                ...
                success: function(response) {
                    responValidasi(['tambah'], ['kode', 'kategori_barang'], response);
                    if (response.sukses) {
        ...
        $(".content").on("click", ".ubah", function() {
            ...
            $("#kategori_barang").val($(this).data("kategori_barang"));
            $("#keterangan").val($(this).data("keterangan"));
            $(".modal-footer").append('<input type="hidden" name="kategori_barang_id" value="' + $(this).data("id") + '">');
        })
        ...
        $(".content").on("click", "#ubah", function(e) {
            ...
            $.ajax({
                type: "post",
                url: `${BASE_URL}/inv/master/kategoribarang/ubah`,
                dataType: "json",
                contentType: false,
                processData: false,
                cache: false,
                data: formData,
                success: function(response) {
                    responValidasi(['ubah'], ['kode', 'kategori_barang'], response);
                    if (response.sukses) {
                        $("#formModal").modal("hide");
                        table.ajax.reload();
                    }
        ...
        $(".content").on("click", ".hapus", function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `${BASE_URL}/inv/master/kategoribarang/hapus`,
                        data: {
                            id: $(this).data("id")
                        },
                        success: function(response) {
